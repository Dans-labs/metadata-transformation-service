        ...
        <xsl:for-each select="//map[@key='metadata']/map">
            <xsl:choose>
                ...
                <xsl:when test="(@key='interviewer')">
                    "<xsl:value-of select="@key"/>": [
                    <xsl:for-each select="./array/map">
                        <xsl:if test="string-length(./map[@key='interviewer_public']/array[@key='value']/string)!=0"> 
                        {
                        <xsl:for-each select="./map">
                            <xsl:if test="@key='interviewer_first_name' or  @key='interviewer_last_name' ">
                            <xsl:choose>
                                <xsl:when test="./map">
                                ...
                                <xsl:if test="@key='interviewer_first_name'">
                                    <xsl:text>,</xsl:text>
                                </xsl:if>
                            </xsl:if>
                        </xsl:for-each>
                ...
                <xsl:when test="(@key='interpreter')">
                    "<xsl:value-of select="@key"/>": [
                    <xsl:for-each select="./array/map">
                        <xsl:if test="string-length(./map[@key='interpreter_public']/array[@key='value']/string)!=0"> 
                            {
                            <xsl:for-each select="./map">
                                <xsl:if test="@key='interpreter_first_name' or  @key='interpreter_last_name' ">
                                    <xsl:choose>
                                        <xsl:when test="./map">
                                            "<xsl:value-of select="@key"/>": "<xsl:value-of select="./map/string[@key='value']"/>"
                                        </xsl:when>
                                    ...
                                    <xsl:if test="@key='interpreter_first_name'">
                                        <xsl:text>,</xsl:text>
                                    </xsl:if>
                                </xsl:if>
                            </xsl:for-each>
                ...
                <xsl:when test="(@key='others')">
                    "<xsl:value-of select="@key"/>": [
                    <xsl:for-each select="./array/map">
                        <xsl:if test="string-length(./map[@key='others_public']/array[@key='value']/string)!=0"> 
                            {
                            <xsl:for-each select="./map">
                                <xsl:if test="@key='others_first_name' or  @key='others_last_name' ">
                                    <xsl:choose>
                                        <xsl:when test="./map">
                                            "<xsl:value-of select="@key"/>": "<xsl:value-of select="./map/string[@key='value']"/>"
                                        </xsl:when>
                                    ...
                                    <xsl:if test="@key='others_first_name'">
                                        <xsl:text>,</xsl:text>
                                    </xsl:if>
                                </xsl:if>
                            </xsl:for-each>
