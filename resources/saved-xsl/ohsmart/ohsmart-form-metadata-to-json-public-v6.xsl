        ...
        <xsl:for-each select="//map[@key='metadata']/map">
            <xsl:choose>
                <xsl:when test="(@key='interviewee')">
                    ...
                    <xsl:for-each select="./array/map">
                        ...
                        <xsl:for-each select="./map">
                            <xsl:if test="./string/following-sibling::boolean[@key='private']='false'">
                                <xsl:choose>
                                    <xsl:when test="./array">
                                        "<xsl:value-of select="@key"/>": "<xsl:value-of select="./array/string"/>"
                                    </xsl:when>
                                    <xsl:otherwise>
                                        <xsl:message><xsl:value-of select="./string/following-sibling::boolean[@key='private']='true'"/></xsl:message>
                ...
                <xsl:when test="(@key='interviewer') or (@key='interpreter') or (@key='others') or (@key='interview_date_time') or (@key='recorded_by') 
                    ...
                    "<xsl:value-of select="@key"/>": [
                    <xsl:for-each select="./array/map">
                        <xsl:variable name="hasPrivateTrue" select="./map/boolean[@key='private']"/>
                        {
                        <xsl:if test="$hasPrivateTrue = 'false'">
                            <xsl:for-each select="./map">
                                <xsl:choose>
                                    <xsl:when test="./map">
