  ...
  "datasetVersion": {
    ...
    "metadataBlocks": {
      ...
      <xsl:for-each select="//map[@key='metadata']/map[@key='audience']/array/map">
        ...
        <xsl:variable name="wikidataKeywords">
        ...
        <xsl:variable name="freetextKeywords">
       ...
       "typeClass": "compound",
       "value": [
        <xsl:if test="normalize-space($aatKeywords)">
            <xsl:value-of select="$aatKeywords"/>
            <xsl:if test="normalize-space($wikidataKeywords) or normalize-space($freetextKeywords)">
                <xsl:text>,</xsl:text>
            </xsl:if>
        </xsl:if>

        <xsl:if test="normalize-space($wikidataKeywords)">
            <xsl:value-of select="$wikidataKeywords"/>
            <xsl:if test="normalize-space($freetextKeywords)">
                <xsl:text>,</xsl:text>
            </xsl:if>
        </xsl:if>

        <xsl:if test="normalize-space($freetextKeywords)">
            <xsl:value-of select="$freetextKeywords"/>
        </xsl:if>
